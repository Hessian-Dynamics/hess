...
from utilities.logger import text_logger
...
class MLFFOptimizationDriver:
    ...
    def initVariables(self):
        """
        Load molecule file, initialize MACE MLFF calculator, and set up optimizer.
        """

        log(f"Loading molecule from: {self.options.input}")
        self.atoms = read(self.options.input)

        log(f"Initializing MACE ({self.options.model}) MLFF...")

        # MACE-OFF23 is pretrained on organic molecules (H, C, N, O, F, P, S, Cl, Br, I)
        self.calc = mace_mp(model=self.options.model, device=self.options.device)
    ...
    def optimize(self):
        ...
        log(f" Starting relaxation (fmax threshold = {self.options.fmax} eV/Å)...")
        traj_req = self.options.traj if self.options.traj else None
        self.optimizer = BFGS(
            self.atoms, 
            trajectory=traj_req,
            logfile=f"{self.jobname}.log"
        )
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str, 
        default=None, 
        help="Path to save output .traj optimization trajectory file (optional)"
    )
